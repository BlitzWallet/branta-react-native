...
import { IBrantaClient, IBrantaService, Destination, DestinationType, Payment, ZKPaymentResult } from "./types.js";
...
export class BrantaService implements IBrantaService {
  ...
  private async _decryptDestinations(
    ...
    for (const dest of destinations) {
      ...
      if (dest.type === 'bitcoin_address') {
        if (!encryptionKey) throw new BrantaPaymentException("Payment is ZK but no destination encryption key was provided.");
  ...
  private _buildVerifyUrl(baseUrl: string, lookupValue: string, keys: Map<string, string>): string {
  ...
  private _getPlainPayments(address: string, options: BrantaClientOptions | null): Promise<Payment[]> {
    const privacy = options?.privacy ?? this._defaultOptions?.privacy;
    if (privacy === 'strict' && !getHashZkType(address)) return Promise.resolve([]);
  ...
  private _normalizeAddress(text: string): string {
    ...
    if (lower.startsWith('lnbc') || lower.startsWith('lntb') || lower.startsWith('lnbcrt') || lower.startsWith('bc1q')) return lower;
    return text;
  }
}
